...
import com.slice.indexer.ui.JavaSrcSearcherLuceneProgressive;
import com.slice.indexer.ui.ProductUI;

/** Query the database using a previously generated Lucene index, using a given query. */
public class LuceneIndexerDatabase implements IQueryDatabase {

	ProductUI _product;
	
	public LuceneIndexerDatabase(ProductUI product) {
		_product = product;
	}
	...
	public IQueryDatabaseResult search(JavaSrcSearchQuery query) {
		...
		LuceneIndexerSearchTask searchTask = new LuceneIndexerSearchTask(query, result, _product);
		searchTask.start();
...
/** A separate thread which acquires the database lock, queries the Lucene index using the query, and returns the result.*/
class LuceneIndexerSearchTask extends Thread implements IDatabaseResultProgess {

	final DefStringQueryDatabaseResultImpl _result;
	final JavaSrcSearchQuery _query;
	final ProductUI _product;
	
	public LuceneIndexerSearchTask(JavaSrcSearchQuery query, DefStringQueryDatabaseResultImpl result, ProductUI product) {
		_result = result;
		_query = query;
		_product =product;
	}
